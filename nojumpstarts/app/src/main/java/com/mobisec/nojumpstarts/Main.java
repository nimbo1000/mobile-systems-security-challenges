package com.mobisec.nojumpstarts;

import android.content.ComponentName;
import android.content.Intent;
import android.util.Base64;
import android.util.Log;

import java.io.BufferedReader;
import java.io.StringReader;
import java.security.KeyFactory;
import java.security.PrivateKey;
import java.security.PublicKey;
import java.security.Signature;
import java.security.spec.PKCS8EncodedKeySpec;
import java.security.spec.X509EncodedKeySpec;

class Main {
    private static final String PRIVKEY = "-----BEGIN RSA PRIVATE KEY-----\\nrfqFhHLw9qfYnmQQXhaXGBhZLQFYMKaYURg5/Z55Xmpmr//mK58m\n-----END RSA PRIVATE KEY-----\n";
    private static final String PUBKEY = "-----BEGIN PUBLIC KEY-----\\nFQIDAQAB\n-----END PUBLIC KEY-----\n";

    Main() {
    }

    public static PKCS8EncodedKeySpec getPrivateKeySpec() throws Exception {
        StringBuilder pkcs8Lines = new StringBuilder();
        BufferedReader rdr = new BufferedReader(new StringReader(PRIVKEY));
        while (true) {
            String readLine = rdr.readLine();
            String line = readLine;
            if (readLine != null) {
                pkcs8Lines.append(line);
            } else {
                String pkcs8Pem = pkcs8Lines.toString();
                String str = BuildConfig.FLAVOR;
                return new PKCS8EncodedKeySpec(Base64.decode(pkcs8Pem.replace("-----BEGIN RSA PRIVATE KEY-----", str).replace("-----END RSA PRIVATE KEY-----", str).replaceAll("\\s+", str), 0));
            }
        }
    }

    public static X509EncodedKeySpec getPublicKeySpec() throws Exception {
        StringBuilder pkcs8Lines = new StringBuilder();
        BufferedReader rdr = new BufferedReader(new StringReader(PUBKEY));
        while (true) {
            String readLine = rdr.readLine();
            String line = readLine;
            if (readLine != null) {
                pkcs8Lines.append(line);
            } else {
                String pkcs8Pem = pkcs8Lines.toString();
                String str = BuildConfig.FLAVOR;
                return new X509EncodedKeySpec(Base64.decode(pkcs8Pem.replace("-----BEGIN PUBLIC KEY-----", str).replace("-----END PUBLIC KEY-----", str).replaceAll("\\s+", str), 0));
            }
        }
    }

    public static PrivateKey getPrivateKey() throws Exception {
        return KeyFactory.getInstance("RSA").generatePrivate(getPrivateKeySpec());
    }

    public static PublicKey getPublicKey() throws Exception {
        return KeyFactory.getInstance("RSA").generatePublic(getPublicKeySpec());
    }

    public static byte[] sign(String msg) throws Exception {
        byte[] msgbytes = msg.getBytes();
        PrivateKey privKey = getPrivateKey();
        Signature s = Signature.getInstance("SHA256withRSA");
        s.initSign(privKey);
        s.update(msgbytes);
        return s.sign();
    }

    public static boolean verify(String msg, byte[] signature) {
        try {
            byte[] msgbytes = msg.getBytes();
            PublicKey pubKey = getPublicKey();
            Signature s = Signature.getInstance("SHA256withRSA");
            s.initVerify(pubKey);
            s.update(msgbytes);
            return s.verify(signature);
        } catch (Exception e) {
            StringBuilder sb = new StringBuilder();
            sb.append("exception when verifying: ");
            sb.append(Log.getStackTraceString(e));
            Log.e("MOBISEC", sb.toString());
            return false;
        }
    }

    public static Intent buildIntent(String src, String dst, String chain) {
        String msg;
        String str = "-to-";
        if (chain == null) {
            try {
                StringBuilder sb = new StringBuilder();
                sb.append(src);
                sb.append(str);
                sb.append(dst);
                msg = sb.toString();
            } catch (Exception e) {
                return null;
            }
        } else {
            StringBuilder sb2 = new StringBuilder();
            sb2.append(chain);
            sb2.append("/");
            sb2.append(src);
            sb2.append(str);
            sb2.append(dst);
            msg = sb2.toString();
        }
        byte[] sign = new byte[0];
        try {
            sign = sign(msg);
        } catch (Exception e) {
            Log.e("MOBISEC", Log.getStackTraceString(e));
        }
        Intent i = new Intent();
        Log.d("MOBISEC", dst);
        String str2 ="";
        if (dst.equals("C")) {
            str2 = "com.mobisec.nojumpstarts";
        }
        else {
            str2 = "com.mobisec.plaku.nojumpstarts";
        }
        StringBuilder sb3 = new StringBuilder();
        sb3.append("com.mobisec.nojumpstarts.");
        sb3.append(dst);
        i.setComponent(new ComponentName(str2, sb3.toString()));
        i.putExtra("authmsg", msg);
        i.putExtra("authsign", sign);
        return i;
    }
}